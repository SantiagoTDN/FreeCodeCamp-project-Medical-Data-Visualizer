...
def draw_heat_map():
    # Clean the data
    df_heat = df_heat = df.loc[(df["ap_lo"] <= df["ap_hi"])
    ...
    ax = sns.heatmap(corr,mask=mask,vmax=0.4,cmap=sns.color_palette("icefire", as_cmap=True),square=True,fmt=".1f",annot=True)
